...
GitHub Actions may hold pipeline secrets such as a Render deploy hook. Runtime secrets belong with the running service. The passenger API datastore is `SQLITE_DATABASE_PATH` (Generation Store). Legacy `DATABASE_URL` / PostGIS reader-role settings are not part of the passenger runtime path; VM cutover may move hosting off Render while keeping the same CI→deploy trigger pattern.
